...
Prediction handling
	...
	•	Multispecies intensity analysis (Python `intensity_fit` backend, set `SPECTRAVISUAL_INTENSITY_FIT_ROOT`): the assigned transitions are written as temporary LIN/CAT/INT files. That model reads every LGINT as a 300 K intensity, so each Pred&Fit row is first referred from its catalogue TEMP to 300 K through the inverse of the model itself: at T = TEMP the fit reproduces SPCAT's row, and the fitted Trot does not depend on the TEMP the catalogue was generated at. Assignments of one species at the same experimental frequency are one blended observation, written on the strongest component with the summed intensity. With Fit Trot off each group keeps its Pred&Fit Trot; concentrations are always fitted.
