...
/* The CAT's QNFMT supplies NQN.  Present the same fields it supplied, without
   assigning labels such as v/F/I in the UI.  Each number occupies exactly the
   three-character field used by a Pickett .lin record: right-alignment of the
   full text alone is not sufficient when individual J/K values change width. */
...
/* Resolve the lower-state label: its concentration is also the factor used
   for an interstate line, so this is the least surprising species tag.  A
   catalogue not generated by Pred&Fit has no project owner and stays clearly
   labelled as external. */
static const char *prediction_species_name(const AppState *state, const PredLine *line,
                                           char *out, size_t size) {
    const PredFitState *pf = &state->predfit;
    if (line->hamiltonian_id <= 0) {
        snprintf(out, size, "external catalogue");
        return out;
    }
    int state_index = line->n_qn > 3 ? line->M1l : 0;
    for (int h = 0; h < pf->n_hamiltonians; h++) {
        if (pf->hamiltonian[h].id != line->hamiltonian_id) continue;
        const PickettSpecies *species = h == pf->active_hamiltonian
                                      ? pf->species : pf->hamiltonian[h].model.species;
        int n_species = h == pf->active_hamiltonian
                      ? pf->n_species : pf->hamiltonian[h].model.n_species;
        for (int i = 0; i < n_species; i++) {
            if (species[i].state_index == state_index) {
                snprintf(out, size, "%s", species[i].name);
                return out;
            }
        }
        break;
    }
    snprintf(out, size, "State %d", state_index);
    return out;
}

/* One transition row inside a card.  The species is a compact visual label;
   the scientific payload (QN and frequency) is monospace and right-aligned,
   so columns do not jump as J or the number of digits changes. */
static void ui_card_row(SDL_Renderer *ren, SDL_Rect r, int i, const AppState *state,
                        const PredLine *p) {
    char buf[128];
    int y = r.y + 22 + i * 19;

    char species[64];
    prediction_species_name(state, p, species, sizeof(species));
    /* Long project names must not consume the QN column in this compact card. */
    if (strlen(species) > 15) snprintf(species + 12, sizeof(species) - 12, "...");
    int tag_w = ui_text_w(UI_FONT_SANS_SM, species) + 14;
    SDL_Rect tag = {r.x + 10, y + 1, tag_w, 17};
    fill_rounded_rect(ren, tag, 3, UI_RAISED);
    ui_text_v(ren, UI_FONT_SANS_SM, species, tag.x + 7, tag, UI_DIM);

    format_pred_qn(buf, sizeof(buf), p);
    int freq_right = r.x + r.w - 10;
    int qn_right = freq_right - ui_text_w(UI_FONT_MONO_SM, "00000.0000") - 16;
    ui_text_right(ren, UI_FONT_MONO_SM, buf, qn_right,
                  y + (19 - ui_text_h(UI_FONT_MONO_SM)) / 2, UI_TEXT);
    fmt_mhz(buf, sizeof(buf), p->freq_mhz, 4);
    ui_text_right(ren, UI_FONT_MONO_SM, buf, freq_right,
...
static void draw_cursor_overlay(SDL_Renderer *ren, TTF_Font *font, AppState *state, Layout *l) {
    ...
    if (state->n_selected > 0) {
        ...
        for (int i = 0; i < show_n; i++) {
            ...
            if (idx >= 0 && idx < state->n_pred) ui_card_row(ren, card, i, state, &state->pred_lines[idx]);
        }
        return;
    }
    ...
    if (state->bar_active && state->n_pred > 0) {
        ...
        if (hover_n > 0 && closest < tol) {
            ...
            for (int i = 0; i < hover_n; i++)
                ui_card_row(ren, card, i, state, &state->pred_lines[hover_idx[i]]);
        }
    }
}
